...
function renderExerciseSelection(){
 modal(`<div class="workout-entry-header"><div class="handle"></div><h2 class="workout-form-title">Select Exercises</h2><div class="muted workout-form-description">Exercises are sorted alphabetically. Select the exercises you performed.</div></div><div class="workout-entry-scroll">${workoutDraft.muscles.map(id=>{let ex=sortedExercisesForMuscle(id);return `<div class="field"><div class="section-title">${esc(muscle(id))}</div>${ex.length?ex.map(e=>`<button class="exercise-row exercise-pick pick ${workoutDraft.exercises.some(x=>x.exerciseId===e.id)?"selected":""}" data-exercise="${e.id}" onclick="this.classList.toggle('selected')"><span>${esc(e.name)}</span><span class="pick-plus"><svg class="icon" aria-hidden="true"><use href="#plus"/></svg></span></button>`).join(""):`<div class="empty">No exercises in this group.</div>`}</div>`}).join("")}</div><div class="modal-actions workout-modal-actions"><button class="primary btn-wide workout-next-button" onclick="continueToSetDetails()">Next: Enter Weight & Reps</button><button class="outline btn-wide workout-cancel-button" onclick="openWorkout(workoutDraft.muscles,workoutDraft.date,true)">Back</button></div>`,"workout-entry-sheet")
 document.body.classList.add("workout-form-open")
}

...
function workoutCard(w){
 ...
 return `<button class="workout-card card" onclick="viewWorkout('${w.id}')"><div class="workout-avatar"><svg class="icon"><use href="#dumbbell"/></svg></div><div class="workout-card-copy"><strong>${esc(w.muscles.map(muscle).join(" + "))}</strong>${date?`<span class="workout-card-date"><svg class="inline-icon"><use href="#calendar-icon"/></svg> ${esc(date)}</span>`:""}<div class="workout-metrics"><b><svg class="inline-icon"><use href="#dumbbell"/></svg> ${ex}</b><b><svg class="inline-icon"><use href="#layers"/></svg> ${sets}</b><b><svg class="inline-icon"><use href="#clock"/></svg> ${durationLabel(dur)}</b></div></div><span class="card-arrow" aria-hidden="true"><svg class="icon"><use href="#chevron-right"/></svg></span></button>`;
}

...
function heroCard(){return `<section class="hero-card"><div class="hero-copy"><h2>Start Workout</h2><p>Log your workout and<br>keep your streak alive!</p><button class="hero-btn" onclick="startNewWorkout()">Start Workout <svg class="icon" aria-hidden="true"><use href="#arrow-right"/></svg></button></div><img src="assets/workout-hero.svg" alt="" class="hero-art"></section>`}
...
function homeInProgress(w){let start=w.startTime||localTimeValue();let elapsed=elapsedFrom(start);let pct=Math.min(100,Math.round(elapsed/90*100));return `<section class="home-status card"><h2>Today's Status</h2><div class="status-main"><div class="status-icon active-state"><svg class="icon"><use href="#activity"/></svg></div><div class="status-copy active-text"><strong>Workout in progress</strong><span>Keep going! You’ve got this.</span></div><button class="outline status-action" onclick="continueWorkout()">Continue Workout</button></div><div class="meter-label"><span>Elapsed</span><b>${elapsed} min</b><span>Target: 90 min</span></div><div class="meter" data-meter-pct="${pct}"><i></i><em data-target-pct="${pct}"></em><span><svg class="icon"><use href="#target"/></svg></span></div><div class="target-badge">${pct}% of your target</div></section>`}
function homeCompleted(w){let dur=durationMinutes(w),pct=dur==null?null:Math.min(100,Math.round(dur/90*100));return `<section class="home-status card"><h2>Today's Status</h2><div class="status-main"><div class="status-icon complete"><svg class="icon"><use href="#check"/></svg></div><div class="status-copy green-text"><strong>Workout completed</strong><span>Great job! You crushed it.</span></div><button class="outline status-action" onclick="viewWorkout('${w.id}')">View Workout</button></div>${dur==null?`<div class="status-note">Duration not recorded</div>`:`<div class="meter-label"><span>Duration</span><b>${durationLabel(dur)}</b><span>Target: 90 min</span></div><div class="meter" data-meter-pct="${pct}"><i></i><span><svg class="icon"><use href="#target"/></svg></span></div><div class="target-badge">${pct}% of your target</div>`}</section>`}

...
function startHomeTimer(){
 ...
 homeTimer=setInterval(()=>{
  ...
  if(active && active.date===dateKey(new Date())) renderHome();
 },60000);
}
function renderHome(){
 const today=new Date();today.setHours(0,0,0,0),k=dateKey(today);
 const active=state.activeWorkout?.date===k?state.activeWorkout:null;
...
function heroCard(){return `<section class="hero-card"><div class="hero-copy"><h2>Start Workout</h2><p>Log your workout and<br>keep your streak alive!</p><button class="hero-btn" onclick="startNewWorkout()">Start Workout <b>→</b></button></div><img src="assets/workout-hero.svg" alt="" class="hero-art"></section>`}
function homeNotStarted(){return `<section class="home-status card"><h2>Today's Status</h2><div class="status-main"><div class="status-icon neutral">▱</div><div class="status-copy"><strong>No workout yet</strong><span>Ready when you are.</span></div><button class="outline status-action" onclick="startNewWorkout()">Start Workout</button></div></section>`}
function homeInProgress(w){let start=w.startTime||localTimeValue();let elapsed=elapsedFrom(start);let pct=Math.min(100,Math.round(elapsed/90*100));return `<section class="home-status card"><h2>Today's Status</h2><div class="status-main"><div class="status-icon green">🏃</div><div class="status-copy green-text"><strong>Workout in progress</strong><span>Keep going! You’ve got this.</span></div><button class="outline status-action" onclick="continueWorkout()">Continue Workout</button></div><div class="meter-label"><span>Elapsed</span><b>${elapsed} min</b><span>Target: 90 min</span></div><div class="meter"><i style="width:${pct}%"></i><em style="left:${pct}%"></em><span>◎</span></div><div class="target-badge">${pct}% of your target</div></section>`}
function homeCompleted(w){let dur=durationMinutes(w),pct=dur==null?0:Math.round(dur/90*100);return `<section class="home-status card"><h2>Today's Status</h2><div class="status-main"><div class="status-icon complete">✓</div><div class="status-copy green-text"><strong>Workout completed 🎉</strong><span>Great job! You crushed it.</span></div><button class="outline status-action" onclick="viewWorkout('${w.id}')">View Workout</button></div><div class="meter-label"><span>Duration</span><b>${durationLabel(dur)}</b><span>Target: 90 min</span></div><div class="meter"><i style="width:${Math.min(100,pct)}%"></i><span>◎</span></div><div class="target-badge">${pct}% of your target</div></section>`}
...
function startNewWorkout(){openWorkout([],dateKey(new Date()),true)}

function weeklyCard(start,count){
 let days=weekDates(start);
 return `<section class="weekly-card card"><div class="section-head"><h2>Weekly Activity</h2><div class="week-select" onclick="openWeekPicker()"><b>#${weekNumber(start)}</b><span>${rangeLabel(start)}</span><i>⌄</i></div></div><div class="week-count"><svg class="icon"><use href="#dumbbell"/></svg><strong>${count} workout${count===1?"":"s"}</strong></div><div class="week-days">${days.map(d=>{let has=state.workouts.some(w=>w.date===dateKey(d)),today=isToday(d);return `<button onclick="goToWeekDay('${dateKey(d)}')" class="week-day"><span>${d.toLocaleDateString("en-IN",{weekday:"short"})}</span><i class="${has?"workout":today?"today":"rest"}">${has?"✓":today?"":"−"}</i></button>`}).join("")}</div><div class="legend"><span><i class="dot workout"></i>Workout</span><span><i class="dot rest"></i>Rest</span><span><i class="dot today"></i>Today</span></div><div class="week-stats"><span>▣ <b>${count} workouts</b></span><span>▱ <b>${days.filter(d=>!state.workouts.some(w=>w.date===dateKey(d))).length} rest days</b></span><span>▣ <b>${remainingDays(days)} day${remainingDays(days)===1?"":"s"} remaining</b></span></div><button class="full-week" onclick="go('workouts')">View full week <b>→</b></button></section>`
}
...
function quickProgress(){let ws=weekStart(new Date()),we=weekEnd(new Date()),w=state.workouts.filter(x=>x.date>=dateKey(ws)&&x.date<=dateKey(we));let volume=0;w.forEach(x=>(x.exercises||[]).forEach(raw=>{let e=normalizedEntry(raw,x.unit||"kg");(e.sets||[]).forEach(s=>volume+=(Number(s.weight)||0)*(Number(s.reps)||0)*(e.unit==="lb"?0.45359237:1))}));return `<section class="quick card"><div class="section-head"><h2>Quick Progress</h2><button onclick="go('progress')">View full progress →</button></div><div class="quick-grid"><div><i>▥</i><b>${w.length}</b><span>Workouts</span><small>This Week</small></div><div><i>▱</i><b>${Math.round(volume).toLocaleString()} kg</b><span>Volume</span><small>This Week</small></div><div><i>↗</i><b>${countPRs()}</b><span>PRs</span><small>This Month</small></div><div><i>◎</i><b>${calcStreak()}</b><span>Day Streak</span><small>Keep it up!</small></div></div></section>`}
function countPRs(){
...
function renderCalendar(){
 ...
 document.getElementById("workoutView").innerHTML=`<section class="workout-week card"><div class="workout-week-head"><button onclick="moveWeek(-1)">‹</button><div><strong>#${weekNumber(start)}</strong><span>${rangeLabel(start)}</span></div><button onclick="moveWeek(1)">›</button></div><div class="workout-days">${days.map(d=>{let k=dateKey(d),has=state.workouts.some(w=>w.date===k),today=isToday(d);return `<button class="${k===sel?"selected ":""}${today?"is-today":""}" onclick="selectWorkoutDay('${k}')"><span>${d.toLocaleDateString("en-IN",{weekday:"short"})}</span><i class="${has?"workout":today?"today":"rest"}">${has?"✓":today?"":"−"}</i><small>${d.getDate()}</small></button>`}).join("")}</div><div class="legend"><span><i class="dot workout"></i>Workout</span><span><i class="dot rest"></i>Rest</span><span><i class="dot today"></i>Today</span></div><button class="week-picker-link" onclick="openWeekPicker()">Select Week ▾</button></section>${renderSelectedDay(sel)}${renderRecentWorkouts()}`;
}
function renderSelectedDay(k){
 let d=new Date(k+"T00:00:00"),ws=state.workouts.filter(w=>w.date===k);
 if(!ws.length)return `<section class="day-section"><div class="selected-date">${d.toLocaleDateString("en-IN",{weekday:"long",month:"short",day:"numeric"})}</div><div class="workout-card rest-card card"><div class="workout-avatar rest-avatar">☁</div><div class="workout-card-copy"><strong>Rest Day</strong><span>Recovery day — let your muscles recover and come back stronger.</span></div></div></section>`;
...
function workoutCard(w){let dur=durationMinutes(w),ex=w.exercises?.length||0,sets=(w.exercises||[]).reduce((n,e)=>n+(e.sets?.length||0),0);return `<button class="workout-card card" onclick="viewWorkout('${w.id}')"><div class="workout-avatar">🏋️</div><div class="workout-card-copy"><strong>${esc(w.muscles.map(muscle).join(" + "))}</strong><span>◫ ${formatTime(w.startTime)} ${w.endTime?`– ${formatTime(w.endTime)}`:""}</span><div class="workout-metrics"><b>♧ ${ex}<small>Exercises</small></b><b>▱ ${sets}<small>Sets</small></b><b>◷ ${durationLabel(dur)}<small>Duration</small></b></div></div><b class="card-arrow">›</b></button>`}
...
function openWeekPicker(){
 ...
 modal(`<div class="handle"></div><div class="picker-title"><h2>Select Week</h2><button onclick="closeModal()">×</button></div><div class="week-list">${items.map(d=>`<button onclick="chooseWeek('${dateKey(d)}')" class="${dateKey(d)===dateKey(selectedWeekStart)?"chosen":""}"><b>#${weekNumber(d)}</b><span>${rangeLabel(d)}</span>${iAmThisWeek(d)?'<em>This Week ✓</em>':""}</button>`).join("")}</div><button class="outline btn-wide" onclick="closeModal()">Close</button>`)
}
...
function chooseExercises(){
 ...
 if(end && end===start){alert("End time must be different from start time.");return}
...
function setRow(exIndex,setIndex,set){const remove=workoutDraft.exercises[exIndex].sets.length>1?`<button class="remove-set" onclick="removeSet(${exIndex},${setIndex})">×</button>`:`<span class="remove-placeholder"></span>`;return `<div class="set-row"><span class="set-number">${setIndex+1}</span><label class="set-field"><span>Weight</span><input class="input compact" type="number" min="0" step="0.1" value="${esc(set.weight??"")}" oninput="updateSet(${exIndex},${setIndex},'weight',this.value)"></label><label class="set-field"><span>Reps</span><input class="input compact" type="number" min="1" step="1" value="${esc(set.reps??"")}" oninput="updateSet(${exIndex},${setIndex},'reps',this.value)"></label>${remove}</div>`}
...
function saveWorkout(){
 ...
 if(!muscles.length||!isValidDateString(date)||!workoutDraft.startTime){alert("Complete the workout date, start time and muscle groups.");return}
 if(!workoutDraft.endTime){alert("Enter the end time to complete the workout.");return}
 if(!exercises.length){alert("Select at least one exercise.");return}
 for(const e of exercises)for(const s of e.sets||[]){let weight=Number(s.weight),reps=Number(s.reps);if(s.weight===""||!Number.isFinite(weight)||weight<0||s.reps===""||!Number.isInteger(reps)||reps<1){alert("Enter valid weight and reps for every set.");return}}
...
function deleteWorkout(id){if(confirm("Delete this workout?")){state.workouts=state.workouts.filter(w=>w.id!==id);save();closeModal();renderCalendar();renderHome()}}

...
function restoreBinWorkout(id){
 ...
 if(item.date&&state.workouts.some(w=>w.date===item.date)){notify(dateTakenMessage());return}
 confirmAction("Restore this workout to your history?",()=>{
  const next=JSON.parse(JSON.stringify(item));
  delete next.deletedAt;
  delete next.muscleNames;
  if(next.date&&state.workouts.some(w=>w.date===next.date&&w.id!==next.id)){notify(dateTakenMessage());return}

...
function renderLibrary(){
 ...
 target.innerHTML=sortedMuscles().map(m=>{
  ...
  return `<div class="library-group"><div class="row"><div class="section-title">${esc(m.name)}</div><div class="actions"><button class="edit" onclick="openExercise('', '${m.id}')"><svg class="icon"><use href="#plus"/></svg> Add</button><button class="edit" onclick="openMuscle('${m.id}')">Edit</button><button class="delete" onclick="deleteMuscle('${m.id}')">Delete</button></div></div>${ex.length?ex.map(e=>`<div class="exercise-row card"><span>${esc(e.name)}</span><span class="actions"><span class="action-spacer" aria-hidden="true"></span><button class="edit" onclick="openExercise('${e.id}')">Edit</button><button class="delete" onclick="deleteExercise('${e.id}')">Delete</button></span></div>`).join(""):`<div class="empty">No exercises in this group.</div>`}</div>`;
 }).join("")+`<div class="section card pad"><button class="primary btn-wide" onclick="openMuscle()"><svg class="icon"><use href="#plus"/></svg> Add New Muscle Group</button></div>`;
}
